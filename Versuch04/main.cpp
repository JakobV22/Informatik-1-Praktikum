...
void horizontRechner(double erdradius, double plattform, double koerpergroesse)
{
	...
	while(true)
	{
		...
		else if (angle < -1* firstAngle/1000000000) {			// wenn angle präziser als 1 Milliardstel des ersten Winkels: break
		...
		for (counter2 = 0;
				richtungFaktor * vektorRadial.winkel(vektorRadial.sub(vektorAuge)) >= richtungFaktor * 90;
				counter2++)									 // bis der Winkel zwischen Radialvektor und Sicht die 90 ° unterschreitet (Richtungsunabhängig)
		{
			vektorRadial.rotiereUmZ(angle);
		}


		std::cout << "Zu weit " << richtung << " gedreht. Ändere Schrittweite von " << angle *(180/M_PI) << " Grad zu " << angle *(-18/M_PI) << " Grad" << std::endl;
